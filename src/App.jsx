...
const ECOSYSTEM_EVENTS_SEED = [
  ...
  { id: "ev7", icon: "📡", color: "#79C0FF", title: "Reid Hoffman at Consensus Miami 2026", sub: "Crypto is the obvious answer for AI agent identity. NFTs are due for a rebirth as identity infrastructure.", ts: Date.now()-1200000 },
...
// Parse agent from /agents/list response shape:
// { agentId, tokenId, name, type, registeredBy, registeredAt, txHash }
function parseAgent(raw, idx) {
  if (!raw || typeof raw !== "object") return null;
  // tokenId is the NFT token ID — this is what we need
...
// Fetch all agents via pagination (max 100 per page)
async function fetchAllAgents() {
  let all = [];
  let cursor = null;
  let hasMore = true;
  let page = 0;
  while (hasMore && page < 15) { // safety cap at 1500 agents
    const url = cursor
...
// Merge: seed agents always present, live agents fill the rest
function mergeAgents(seed, live) {
  const liveIds = new Set(live.map(a => a.id));
  const missing = seed.filter(s => !liveIds.has(s.id));
  return [...live, ...missing];
}
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        {/* Stats bar */}
        <div style={{padding:"16px 24px",borderBottom:"1px solid #ffffff06",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
          {[
        ...
        {/* X accounts — clean link cards */}
        <div style={{padding:"20px 24px",borderBottom:"1px solid #ffffff06"}}>
          <div style={{fontSize:9,color:"#ffffff20",fontFamily:"'Space Mono',monospace",letterSpacing:"0.14em",marginBottom:14}}>FOLLOW ON X</div>
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            {X_ACCOUNTS.map(acc=>(
              <a key={acc.handle} href={`https://x.com/${acc.handle}`} target="_blank" rel="noreferrer"
                style={{textDecoration:"none",flex:"1 1 160px"}}>
                <div style={{border:`1px solid ${acc.color}20`,background:`${acc.color}06`,padding:"12px 14px",transition:"all 0.2s",cursor:"pointer"}}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                    <span style={{fontSize:15,color:acc.color}}>𝕏</span>
                    <span style={{fontSize:12,color:acc.color,fontFamily:"'Space Mono',monospace"}}>{acc.label}</span>
                    <span style={{marginLeft:"auto",fontSize:10,color:acc.color+"60"}}>↗</span>
                  </div>
                  <div style={{fontSize:11,color:"#ffffff28",fontFamily:"'DM Sans',sans-serif"}}>{acc.desc}</div>
                </div>
              </a>
            ))}
          </div>
        ...
        <div style={{padding:"20px 24px 8px"}}>
          ...
          {events.map(ev=>(
            <div key={ev.id} style={{display:"flex",gap:12,padding:"12px 0",borderBottom:"1px solid #ffffff05"}}>
              <div style={{width:32,height:32,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,background:"#ffffff04",border:`1px solid ${ev.color}20`}}>{ev.icon}</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,color:"#CECEE0",fontFamily:"'DM Sans',sans-serif",marginBottom:3,lineHeight:1.4}}>{ev.title}</div>
                <div style={{fontSize:11,color:"#ffffff30",fontFamily:"'DM Sans',sans-serif",lineHeight:1.4,marginBottom:ev.url?6:0}}>{ev.sub}</div>
                {ev.url&&<a href={ev.url} target="_blank" rel="noreferrer" style={{fontSize:10,color:ev.color+"80",fontFamily:"'Space Mono',monospace",textDecoration:"none",letterSpacing:"0.05em"}}>Open →</a>}
              </div>
              <div style={{fontSize:10,color:"#ffffff18",fontFamily:"monospace",flexShrink:0,paddingTop:2}}>{fmtTime(ev.ts)}</div>
            </div>
          ))}
        </div>
        ...
        {liveAgents.length > 0 && (
          <div style={{padding:"8px 24px 24px"}}>
            <div style={{fontSize:9,color:"#ffffff20",fontFamily:"'Space Mono',monospace",letterSpacing:"0.14em",marginBottom:14}}>
              AWAKENED AGENTS — {liveAgents.length}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {liveAgents.slice(0,30).map(agent=>(
                <div key={agent.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:"#ffffff03",border:"1px solid #ffffff06"}}>
                  ...
                  <span style={{fontSize:9,color:"#7DF9AA40",fontFamily:"'Space Mono',monospace",flexShrink:0}}>awakened</span>
                </div>
              ))}
              {liveAgents.length > 30 && (
                <div style={{padding:"8px 12px",textAlign:"center",fontSize:10,color:"#ffffff20",fontFamily:"'Space Mono',monospace"}}>
                  +{liveAgents.length - 30} more awakened agents
                </div>
              )}
            </div>
          </div>
...
function GifPicker({onSelect,onClose}) {
  ...
  const search=useCallback(async qr=>{
    setLoading(true);
    try{ const r=await fetch(`/api/gifs?q=${encodeURIComponent(qr||"crypto")}&limit=24`); const d=await r.json(); setGifs(d.results||[]); }
    catch{ setGifs([]); }
    setLoading(false);
  },[]);
...
export default function NormiesCommons() {
  ...
  const [ecosystemEvents,setEcoEvents] = useState([...ECOSYSTEM_EVENTS_SEED]);
  ...
  useEffect(()=>{
    ...
    fetch("https://api.normies.art/agents/count")
      .then(r=>r.json())
      .then(d=>{ if(d?.count){ setEcoStats(s=>({...s,awakened:d.count.toLocaleString()})); setAgentCount(d.count); } })
      .catch(()=>{});

    // Stats
    fetch("https://api.normies.art/history/stats")
      .then(r=>r.json())
      .then(d=>{ if(d) setEcoStats(s=>({...s,burned:d.burned?d.burned.toLocaleString():s.burned,transforms:d.transforms?d.transforms.toLocaleString():s.transforms})); })
      .catch(()=>{});

    // Floor
    fetch("/api/floor")
      .then(r=>r.json())
      .then(d=>{ if(d?.floor){ setEcoStats(s=>({...s,floor:d.floor})); setEcoEvents(prev=>prev.map(e=>e.id==="ev5"?{...e,title:`Floor: ${d.floor} · live`}:e)); } })
      .catch(()=>{});

    // All agents via pagination
    fetchAllAgents()
      .then(list=>{
        if(!list||list.length===0)return;
        const mapped = list.map((a,i)=>parseAgent(a,i)).filter(Boolean);
        if(mapped.length===0)return;
        // Merge with seed so named agents always resolve
        setAllAgents(mergeAgents(SEED_AGENTS, mapped));
        // Inject recent awakenings into ecosystem feed
        const newEvs = mapped.slice(0,8).map((a,i)=>({
      ...
      <div style={{height:50,borderBottom:"1px solid #ffffff07",display:"flex",alignItems:"center",background:"#07070D",flexShrink:0,zIndex:10}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14,padding:"0 18px",height:"100%",borderLeft:"1px solid #ffffff07"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{fontSize:10,color:"#ffffff25",fontFamily:"monospace"}}>
              {agentCount !== null ? agentCount.toLocaleString() : ecosystemStats.awakened} awakened
            </span>
          </div>
